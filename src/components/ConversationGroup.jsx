...
const ConversationGroup = () => {
  ...
  const [storedData, setStoredData]  = useState(JSON.parse(localStorage.getItem("conversations")));
  ...
  useEffect(() => {
    ...
    newSocketGroup.onopen = () => {
      console.log("WebSocket connected => GROUP");
    };

    newSocketGroup.onmessage = (event) => {
      const data = event.data;
      console.log("Received data Group:", data);

    };
    
    newSocketGroup.onclose = () => {
      console.log("WebSocket close => GROUP");
    };

    newSocketGroup.onerror = (event) => {
  ...
  }, [reloadCounter, id, tokenFromCookies, startIndex, endIndex, group, cons])
  // ============================================================================================================================
...
const handleDeleteGroup = async () => {
  ...
  if (socketGroup && user) {
    ...
    // let updatedItems = storedData.filter(item => item.id !== id);
    // setStoredData(updatedItems);

    // localStorage.setItem("conversations", storedData)
    // setCons(storedData)
    navigate("/App")
  } else {
    console.error("WebSocket is not initialized.");
...
const deleteMember = async (member) => {
  ...
  console.log(socketGroup.readyState);
  ...
  if (socketGroup) {
    ...
    if (socketGroup.readyState === WebSocket.OPEN) {
      socketGroup.send(JSON.stringify(outGroup));
      console.log("open");
    } else {
      console.log("WebSocket is not open. Current state: " + socketGroup.readyState);
    }
              ...
              <div className="flex flex-col items-center h-screen">
                ...
                <div className="flex flex-col justify-end h-full bg-white ">
                  ...
                  <div className="mb-4 flex-auto border-b-4 border-gray-200 px-4">
                    ...
                    <p className="text-gray-600"><FontAwesomeIcon icon={faUsers} /> {group.members.length+group.admin.length+1} thành viên</p> 
